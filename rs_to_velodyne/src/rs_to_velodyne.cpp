...
int main(int argc, char **argv) {
    ...
    if (argc < 3) {
        ...
        if (std::strcmp("XYZI", argv[1]) == 0) {
            subRobosensePC = nh.subscribe("/rslidar_points_init", 1, rsHandler_XYZI);
        } else if (std::strcmp("XYZIRT", argv[1]) == 0) {
            subRobosensePC = nh.subscribe("/rslidar_points_init", 1, rsHandler_XYZIRT);
        } else {
            ROS_ERROR(argv[1]);
    ...
    ROS_INFO("Listening to /rslidar_points_init ......");
    ros::spin();
    return 0;
}
